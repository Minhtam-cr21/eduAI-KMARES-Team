...
export default async function StudentProfilePage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select(
      "id, role, full_name, created_at, goal, hours_per_day, preferred_learning, birth_year, school, class, mbti_type, learning_style, career_orientation, assessment_completed, strengths, weaknesses"
    )
    .eq("id", user.id)
    .maybeSingle();
